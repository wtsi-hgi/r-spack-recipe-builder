...
class UvPackage(PythonPackage):
    """Specialized class for packages that are built using uv."""

    # To be used in UI queries that require to know which
    # build-system class we are using
    build_system_class = "UvPackage"
    #: Legacy buildsystem attribute used to deserialize and install old specs
    legacy_buildsystem = "python_uv"

    #: Callback names for install-time test
    install_time_test_callbacks = ["test"]

    build_system("python_uv", "python_pip")

    with spack.multimethod.when("build_system=python_uv"):
        extends("python")
        depends_on("py-wheel", type="build")
        depends_on("py-setuptools", type="build")


@spack.builder.builder("python_uv")
class PythonUvBuilder(BaseBuilder):
    phases = ("install",)
